...
def extract_text_to_markdown(base_dir, output_file="context.md"):
    ...
    target_directories = [
        os.path.join(base_dir, "bohe_backend", "alembic"),
        os.path.join(base_dir, "bohe_backend", "models"),
        os.path.join(base_dir, "bohe_backend", "scripts"),
        os.path.join(base_dir, "bohe_backend", "views"),
        os.path.join(base_dir, "bohe_backend")
    ]

    bohe_backend_root = os.path.join(base_dir, "bohe_backend")
    if not os.path.isdir(bohe_backend_root):
        print(f"Error: Direktori '{bohe_backend_root}' tidak ditemukan.")
        return
    ...
    with open(output_path, 'w', encoding='utf-8') as outfile:
        ...
        print(f"Memproses file-file langsung di direktori: {bohe_backend_root}")
        for item in os.listdir(bohe_backend_root):
            item_path = os.path.join(bohe_backend_root, item)
